...
def main():
    try:
        ...
        character = input(
            "Enter characters (e.g. hatsune_miku, kagamine_rin, etc.): "
        ).strip()

        pages_input = input(
            "Enter pages (e.g. 1 3 5 or 1-5 | default is 1 page): "
        ).strip()
        if not pages_input:
            pages = [1]
        else:
        ...
        folder_name = input(
            "Enter folder name (or press Enter for default 'images'): "
        ).strip()
        if not folder_name:
            folder_name = "images"
        ...
        print(" ")
        slowprint("downloading....")
        get_images(tag, character, pages, folder_name)

        slowprint("Download completed successfully!")
